...
1. `image_reuse_network.html`: Filterable media reuse table and perceptual network explorer answering all 8 forensic questions.
2. `text_similarity_network.html`: Side-by-side title comparison and distinctive OCR phrase inspector.
3. `geographic_relationship_map.html`: Inter-city corridor volume rankings and jurisdiction matrix.
4. `relationship_graph.html`: Master interactive Vis.js graph explorer with component selection, listing search, and evidence inspector.
